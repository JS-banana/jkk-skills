#!/usr/bin/env python3
"""Validate the md-preview skill package and rendered HTML."""
...
def validate_skill(skill_dir: Path) -> tuple[list[str], list[str]]:
    ...
    if skill_md.is_file():
        ...
        if not re.search(r"^name:\s*md-preview\s*$", text, flags=re.MULTILINE):
            errors.append("SKILL.md name must be md-preview")
...
def run_self_test() -> tuple[list[str], list[str]]:
    errors: list[str] = []
    warnings: list[str] = []
    with tempfile.TemporaryDirectory(prefix="md-preview-") as temp:
        root = Path(temp)
        source = root / "demo.md"
...
def print_result(errors: list[str], warnings: list[str]) -> int:
    ...
    if errors:
        for error in errors:
            print(f"FAIL {error}")
        return 1
    print("PASS md-preview validation")
    return 0
